...
//	protected $active;
	...
	public function init() {
		...
		if ( class_exists( 'Storefront_Pro' ) && class_exists( 'WooCommerce' ) ) {
			...
			add_action( 'storefront_handheld_footer_bar_links', array( $this, 'footer_bar_link' ) );
		}
	}
	...
	public function wp() {
		...
		$this->product_icon_pos = $this->get( 'sfwl-product-pos' );
		$this->shop_icon_pos    = $this->get( 'sfwl-shop-pos' );
		...
		if ( is_product() ) {
			switch( $this->product_icon_pos ) {
				...
				case 'fr-cart':
					...
					break;
				default:
					add_filter( 'wp_footer', array( $this, 'title_add2wishlist' ) );
			}
		}

		add_filter( 'woocommerce_loop_add_to_cart_link', array( $this, 'a2c_add2wishlist' ) );
		add_filter( 'body_class', array( $this, 'body_class' ) ); //Enqueue styles n scripts
	}

	/**
	 * Adds wishlist icon position class
	 * @param array $classes
	 * @return array Body classes
	 */
	public function body_class( array $classes ) {
		$pos = $this->shop_icon_pos ? $this->shop_icon_pos : 'left-a2c';
		$classes[] = "sfwl-shop-icon-$pos";
...
CSS;
	...
	public function a2w_btn() {
		...
		ob_start();
		?>
		<a href='javascript:void(0)' onclick='sfwl.a2w( <?php echo str_replace( "'", '&#039;', json_encode( $product_attrs ) ); ?>, this )'
			 class="sfwl-a2w sfwl-a2w-<?php echo $id ?>">
		...
		<script>
			!function () {
				...
				document.querySelectorAll( '.product .entry-title, .product_title' ).forEach( function( el ) {
					...
					el.innerHTML = <?php echo $this->product_icon_pos ? 'btn + title' : 'title + btn'; ?>;
				} );
			}();
		</script>
		<?php
	 ...
	 */
	public function footer_bar_link( $links ) {
		$wishlist = [
			'wishlist' => [
				'callback' => array( $this, 'footer_bar_link_callback' ),
			]
		];
		switch ( $this->get( 'sfwl-app-icon-pos' ) ) {
			case 'right':
				$links = $links + $wishlist;
				break;
			case 'hide':
				break;
			default:
	 ...
	 * @action pootlepb_le_dialogs
	 */
	public function footer_bar_link_callback() {
		?>
		<a href="#wishlist" class="fa fa-heart">
			<?php _e( 'Wishlist', 'sfwl' ); ?>
		</a>
		<?php
	}
	...
	function fields( $fields ) {
		return array_merge( $fields, array(
			...
			array(
				'id'          => 'sfwl-hide-labels',
				'label'       => 'Show icons without labels',
				'section'     => 'Wishlist',
				'priority'    => 20,
				'type'        => 'checkbox',
			),
